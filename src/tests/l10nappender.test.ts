...
describe('l10nappender', () => {
    ...
    it('registers new translations', async () => {
        ...
        try {
            let trans = await loadThemeTranslations()
    ...
    it('does reject on network error', async () => {
        try {
            let trans = await loadThemeTranslations('networkissue')
    ...
    it('does reject on server error', async () => {
        try {
            let trans = await loadThemeTranslations('500')
            expect('').toBe('Unexpected pass')
        } catch (e) {
            expect(e instanceof Error).toBe(true)
    ...
    it('does reject on unavailable bundle', async () => {
        try {
            let trans = await loadThemeTranslations('404')
            expect('').toBe('Unexpected pass')
        } catch (e) {
            expect(e instanceof Error).toBe(true)
    ...
    it('does reject on invalid bundle', async () => {
        try {
            let trans = await loadThemeTranslations('noappend')
    ...
    it('does reject on missing bundle', async () => {
        const callback = jest.fn()
        try {
            let trans = await loadThemeTranslations('invalidappend')
            expect('').toBe('Unexpected pass')
        } catch (e) {
            expect(e instanceof Error).toBe(true)
    ...
    it('does reject on empty response', async () => {
        const callback = jest.fn()
        try {
            let trans = await loadThemeTranslations('emptyappend')
            expect('').toBe('Unexpected pass')
        } catch (e) {
            expect(e instanceof Error).toBe(true)
            expect((<Error>e).message).toMatch('file format problem')
